    ...
    // ── Căn chỉnh vị trí Khối DANH SÁCH TẬP ngang hàng chuẩn với Top Phim Tuần Này ──
    function alignEpisodesWithTopWeekly() {
        const epSection = document.getElementById('episodes-section');
        if (!epSection) return;

        if (window.innerWidth < 1024) {
            epSection.style.marginTop = '24px';
            return;
        }

        const playerBox = document.getElementById('main-player-box');
        const topWeeklyHeader = document.querySelector('.ap-top-weekly-header') || document.querySelector('.ap-top-weekly-title') || document.getElementById('top-weekly-sidebar');

        if (!playerBox) {
            epSection.style.marginTop = '36px';
            return;
        }

        if (topWeeklyHeader) {
            requestAnimationFrame(() => {
                const targetTop = topWeeklyHeader.getBoundingClientRect().top + window.scrollY;
                const playerBottom = playerBox.getBoundingClientRect().bottom + window.scrollY;
                const diff = targetTop - playerBottom;
                if (diff >= 20) {
                    epSection.style.marginTop = Math.round(diff) + 'px';
                } else {
                    epSection.style.marginTop = '32px';
                }
            });
        } else {
            epSection.style.marginTop = '32px';
    ...
    window.alignEpisodesWithTopWeekly = alignEpisodesWithTopWeekly;
    window.addEventListener('resize', alignEpisodesWithTopWeekly);
    window.addEventListener('load', () => {
        alignEpisodesWithTopWeekly();
        setTimeout(alignEpisodesWithTopWeekly, 300);
        setTimeout(alignEpisodesWithTopWeekly, 1000);
        setTimeout(alignEpisodesWithTopWeekly, 2500);
    });
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', alignEpisodesWithTopWeekly);
    } else {
        alignEpisodesWithTopWeekly();
        setTimeout(alignEpisodesWithTopWeekly, 300);
        setTimeout(alignEpisodesWithTopWeekly, 1000);
