...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (!slug) {
        try {
            slug = sessionStorage.getItem('aphim_last_watched_slug') || localStorage.getItem('aphim_last_watched_slug');
            if (slug) {
                episodeSlug = sessionStorage.getItem('aphim_last_watched_ep') || episodeSlug || 'tap-1';
            }
        } catch(e) {}
        if (!slug) {
...
function updateWatchUrlState(movie, episode, serverIndex) {
    ...
    document.title = `Xem Phim ${movie.name} ${epStr ? epStr + ' ' : ''}[${variantTag}] - APhim`;
}
...
function renderMovieInfo(movie, episode) {
    ...
    if (typeof SEO !== 'undefined') {
        ...
        document.title = `Xem Phim ${movie.name} ${epName ? '- ' + epName : ''} Full HD | APhim`;
    }
