/**
 * APhim Core API Engine (Multi-Source API Merging & Ultra-Fast Image Loading Engine)
...
(function() {
    ...
    // 🎯 URL GENERATORS FOR MOVIE DETAILS & WATCH PAGE (100% PURE HTML COMPLIANT)
    window.APhimCore.getMovieDetailUrl = function(slug) {
        if (!slug) return '#';
        return 'movie-detail.html?slug=' + encodeURIComponent(slug);
    };

    window.APhimCore.getWatchUrl = function(slug, ep = 'tap-1') {
        if (!slug) return '#';
        return 'watch.html?slug=' + encodeURIComponent(slug) + '&episode=' + encodeURIComponent(ep);
    };
    ...
    window.APhimCore.createGridCardHTML = function(movie) {
        ...
        const detailUrl = window.APhimCore.getMovieDetailUrl ? window.APhimCore.getMovieDetailUrl(movie.slug) : ('movie-detail.html?slug=' + encodeURIComponent(movie.slug));

        return `
            <a href="${detailUrl}" title="${movie.name}" class="grid-movie-card">
    ...
    window.APhimCore.fetchMovieGridData = async function(type, paramValue, page = 1) {
        ...
        const isLocalNodeServer = typeof window !== 'undefined' && 
            (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && 
            (window.location.port === '3000' || window.location.port === '5000');
            
        const urlsToTry = isLocalNodeServer ? [
            endpointPath,
            'https://phimapi.com' + endpointPath,
            'https://ophim1.com' + endpointPath
        ] : [
            'https://phimapi.com' + endpointPath,
            'https://ophim1.com' + endpointPath,
            endpointPath
